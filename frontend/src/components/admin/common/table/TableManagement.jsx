...
const TableManagement = ({
  ...
  const renderCustomCell = (column, item) => {
    switch (column.key) {
      case "status": {
        // Thêm {} để tạo block scope
        const statusMap = {
          ACTIVE: { color: "success", text: "Hoạt động" },
          PUBLISHED: { color: "success", text: "Đã xuất bản" },
          REJECTED: { color: "error", text: "Đã từ chối" },
          DISABLED: { color: "default", text: "Vô hiệu hóa" },
          PENDING: { color: "warning", text: "Đang chờ duyệt" },
        ...
        return (
          <span
            className={`px-2 py-1 rounded text-xs font-semibold bg-${status.color}-100 text-${status.color}-800`}
          >
            {status.text}
          </span>
        );
